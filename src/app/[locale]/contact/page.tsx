...
const TOPIC_KEYS = ["general", "support", "reciter", "partnership", "press", "careers"] as const;
...
export default function ContactPage() {
  ...
  const topics = TOPIC_KEYS.map((key) => ({ value: key, label: t(`topics.${key}`) }));
        ...
        <div className="max-w-6xl mx-auto px-6 lg:px-8">
          ...
          <div className="grid lg:grid-cols-5 gap-12">
            ...
            <div className="lg:col-span-3">
              {sent ? (
                <div className="rounded-2xl border border-emerald-100 dark:border-emerald-800/30 bg-emerald-50 dark:bg-emerald-900/10 p-10 text-center">
                  ...
                  <h3 className="text-slate-900 dark:text-white font-semibold text-lg mb-2">{t("success.title")}</h3>
            ...
            <div className="lg:col-span-2 space-y-6">
              <div>
                <h2 className="text-slate-900 dark:text-white font-semibold mb-5">{t("otherWays")}</h2>
                <div className="space-y-3">
                  {contactCards.map((c) => (
                    ...
                    >
                      ...
                      <div>
                        <p className="text-slate-500 dark:text-slate-400 text-xs mb-0.5">{t(`cards.${c.key}`)}</p>
                        <p className="text-slate-800 dark:text-white text-sm font-medium group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">{c.value}</p>
                      </div>
                    </a>
                  ))}
                </div>
              ...
              <div className="rounded-2xl border border-slate-100 dark:border-white/8 bg-slate-50 dark:bg-[#0D1525] p-5">
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="text-slate-700 dark:text-slate-300 text-sm font-medium mb-1">{t("responseTime.label")}</p>
