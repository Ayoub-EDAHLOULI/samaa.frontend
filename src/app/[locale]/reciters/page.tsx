    ...
    <>
      ...
      <main className="min-h-screen bg-white dark:bg-slate-950 pt-24 pb-16">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <h2 className="text-xs font-bold tracking-widest uppercase text-emerald-500 dark:text-emerald-400 mb-5">{t("popularLabel")}</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-12">
            {popular.map((r) => (
              <div key={r.name} className="flex items-center gap-4 rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-sm hover:shadow-md hover:border-emerald-100 dark:hover:border-emerald-900/50 transition-all duration-200 cursor-default">
                <div className="w-12 h-12 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/30 flex items-center justify-center text-2xl flex-shrink-0">
                  {r.flag}
                </div>
          ...
          <h2 className="text-xs font-bold tracking-widest uppercase text-slate-400 dark:text-slate-500 mb-5">{t("allLabel")}</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {rest.map((r) => (
              <div key={r.name} className="flex items-center gap-3 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 p-3 hover:border-emerald-100 dark:hover:border-emerald-900/40 transition-colors duration-200 cursor-default">
          ...
          <p className="text-center text-slate-400 dark:text-slate-500 text-sm mt-12">{t("moreText")}</p>
        </div>
      </main>
      <Footer />
    </>
