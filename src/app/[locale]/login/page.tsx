...
import { createLoginSchema, LoginFormValues } from "@/validations/auth.schema";
import { getLocaleFromPathname } from "@/api/utils";

const BARS = [3, 5, 8, 6, 11, 14, 10, 7, 13, 17, 21, 16, 18, 14, 10, 7, 5, 8, 12, 9, 6, 11, 15, 7];

function WaveformDecor() {
  return (
    <div className="flex items-end gap-0.75 opacity-30">
      {BARS.map((h, i) => (
        <div key={i} className="rounded-full bg-emerald-400" style={{ width: "3px", height: `${h}px` }} />
      ))}
    </div>
  );
}
...
export default function LoginPage() {
  ...
  const schema = useMemo(
    () => createLoginSchema((key) => t(key as Parameters<typeof t>[0])),
    [t],
  );
  ...
  const onSubmit = async (values: LoginFormValues) => {
    try {
      ...
      notify(t("toastSuccess"), "success");
      const locale = getLocaleFromPathname(pathname);
      // Proxy middleware will redirect to the correct dashboard on next load;
      // push to home so it triggers immediately
  ...
  const inputBase =
    "w-full px-4 py-3 rounded-xl border bg-white dark:bg-white/5 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-white/25 text-sm focus:outline-none focus:ring-2 transition-all duration-200";
  const inputNormal = `${inputBase} border-slate-200 dark:border-white/10 focus:ring-emerald-500/30 focus:border-emerald-400 dark:focus:border-emerald-500`;
  const inputErr = `${inputBase} border-red-400 dark:border-red-500/60 focus:ring-red-400/30 focus:border-red-400`;
      ...
      <div className="hidden lg:flex lg:w-[52%] relative bg-[#070E1B] flex-col justify-between p-12 overflow-hidden">
        ...
        <div className="relative z-10">
          <Link href="/" className="inline-flex items-center gap-3">
            ...
            <span className="text-white font-bold text-xl tracking-tight">Samaa</span>
          </Link>
        </div>
      ...
      <div className="flex-1 flex flex-col justify-center bg-white dark:bg-[#070E1B] px-6 py-12 lg:px-16">
        ...
        <div className="lg:hidden mb-10">
          <Link href="/" className="inline-flex items-center gap-2.5">
            ...
            <span className="text-slate-900 dark:text-white font-bold text-lg">Samaa</span>
          </Link>
        </div>
        ...
        >
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••"
                  {...register("password")}
                  className={`${errors.password ? inputErr : inputNormal} pr-12`}
                />
                <button
                  type="button"
          ...
          <div className="flex items-center gap-4 my-7">
            ...
            <span className="text-slate-400 dark:text-white/20 text-xs">{t("or")}</span>
            <div className="flex-1 h-px bg-slate-100 dark:bg-white/8" />
          </div>
