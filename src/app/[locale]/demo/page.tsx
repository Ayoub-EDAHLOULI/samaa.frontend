...
type DemoState = "idle" | "recording" | "processing" | "match" | "nomatch" | "error";
...
function getSupportedMimeType(): string | null {
  ...
  const candidates = ["audio/mp4", "audio/wav"];
...
export default function DemoPage() {
  ...
  const startRecording = async () => {
    ...
    if (!mimeType) {
      ...
      setErrorMsg("Your browser doesn't support a compatible audio format. Please use Chrome, Safari, or Firefox.");
      setState("error");
      return;
    }
        ...
        <div className="max-w-2xl mx-auto px-6 lg:px-8 text-center">
          ...
          <div className="rounded-3xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 p-12 flex flex-col items-center gap-6">
            ...
            <div className="relative">
              ...
              <button
                onClick={state === "idle" || state === "match" || state === "nomatch" || state === "error" ? startRecording : undefined}
            ...
            <div>
              ...
              {state === "processing" && (
                <>
                  <p className="text-slate-900 dark:text-white font-semibold">Identifying reciter…</p>
                  <p className="text-slate-400 dark:text-slate-500 text-sm mt-1">Analysing audio with AI</p>
                </>
              )}
            ...
            <div className="w-full max-w-xs rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950 p-4 text-left">
              ...
              {state === "match" && result?.reciter && (
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-xl bg-slate-100 dark:bg-slate-800 overflow-hidden flex-shrink-0 relative">
              ...
              {state === "nomatch" && (
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 text-sm">
                    {result?.message ?? "No reciter identified. Try holding your device closer to the audio source."}
                  </p>
                </div>
              )}
          ...
          <p className="text-slate-400 dark:text-slate-500 text-sm mt-6">
            {t("tipPrefix")}{" "}
            <a href="#download" className="text-emerald-500 hover:text-emerald-600 font-medium">
              {t("tipLink")}
            </a>{" "}
            {t("tipSuffix")}
          </p>
